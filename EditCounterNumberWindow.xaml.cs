...
namespace CounterPlg {
    public partial class EditCounterNumberWindow : Window, IDataErrorInfo {
        ...
        public EditCounterNumberWindow(CounterNumber numberToEdit)
        {
            InitializeComponent();
        ...
        public string this[string columnName]
        {
            ...
            {
                string error = null;
                if (columnName == nameof(EditedNumber.Number))
                {
                    if (string.IsNullOrWhiteSpace(EditedNumber.Number))
                    {
                        error = "Введіть номер лічильника.";
                    }
                    else if (!long.TryParse(EditedNumber.Number, out long numberValue) || numberValue < 0)
                    {
                    ...
                    else
                    {
                        ...
                        var existingNumber = _dbContext.CounterNumbers
                            .FirstOrDefault(n => n.Number == EditedNumber.Number && n.Id != EditedNumber.Id);
                        if (existingNumber != null)
                        {
                            error = $"Номер '{EditedNumber.Number}' уже існує.";
                        }
                    }
                }
                return error;
        ...
        {
            // Перевірка номера
            if (!string.IsNullOrWhiteSpace(this[nameof(EditedNumber.Number)]))
            {
                MessageBox.Show(this[nameof(EditedNumber.Number)], "Помилка", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }

            if (EditedNumber.IsWrittenOff)
            {
                if (string.IsNullOrWhiteSpace(EditedNumber.PersonalAccount))
                {
                    MessageBox.Show("Введіть особовий рахунок для списаного номера.", "Помилка", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }

                if (string.IsNullOrWhiteSpace(EditedNumber.Address))
                {
                    MessageBox.Show("Введіть адресу для списаного номера.", "Помилка", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }

                if (string.IsNullOrWhiteSpace(EditedNumber.SelectedMonth) || string.IsNullOrWhiteSpace(EditedNumber.SelectedYear))
                {
                    MessageBox.Show("Виберіть місяць і рік для списаного номера.", "Помилка", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
            ...
            {
                // Якщо номер не списаний, очищаємо дату списання
                EditedNumber.SelectedMonth = null;
                EditedNumber.SelectedYear = null;
                // PersonalAccount і Address можуть бути порожніми
            }
